...
def format_issue_output(issue_type, issue_id, status,
    ...
    assignee = (
        getattr(assignee, "displayName", assignee)
        if assignee
        else "NOT ASSIGNED!!!"
    )
    if len(components) == 0:
...
def get_credentials(cmd):
    config = load_configuration()
    return (
        (config["jira"].get("token") if "jira" in config.sections() else None)
        or os.environ.get("APACHE_JIRA_TOKEN")
        or cmd.prompt(
            "Env APACHE_JIRA_TOKEN not set, "
